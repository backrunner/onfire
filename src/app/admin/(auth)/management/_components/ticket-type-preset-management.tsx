...
export function TicketTypePresetManagement({ tenantId }: { tenantId?: string }) {
  ...
  const [form, setForm] = useState({ tenantId: "", parentId: ROOT, name: "", description: "", sortOrder: "0" });
  ...
  const openEdit = (item: PresetView) => {
    setEditing(item);
    setForm({ tenantId: item.tenantId, parentId: item.parentId ?? ROOT, name: item.name, description: item.description ?? "", sortOrder: String(item.sortOrder) });
    setDialogOpen(true);
  };
  const openApply = () => {
  ...
  const save = async () => {
    ...
    setPending(true);
    try {
      const payload = { ...(!editing && { tenantId: effectiveTenantId }), parentId: form.parentId === ROOT ? null : form.parentId, name: form.name.trim(), description: form.description.trim() || null, sortOrder: Number(form.sortOrder) || 0 };
