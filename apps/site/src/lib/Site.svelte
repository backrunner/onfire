...
<DocsApp {...data} {themeComponents} {loadSearch}>
  ...
  <svelte:fragment slot="doc-header" let:page let:breadcrumbs>
    <header class="onfire-doc-header">
      <nav class="onfire-breadcrumbs" aria-label={page.locale === 'zh' ? '面包屑导航' : 'Breadcrumb'}>
        {#each breadcrumbs as item, i}
          {#if i > 0}<span aria-hidden="true">/</span>{/if}
          <a href={item.path}>{item.label}</a>
        {/each}
      </nav>
      <div class="onfire-doc-label">{page.locale === 'zh' ? 'ONFIRE / 使用手册' : 'ONFIRE / HANDBOOK'}</div>
      <h1>{page.title}</h1>
