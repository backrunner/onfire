...
export function InstallPage({ onFinished }: { onFinished: () => void }) {
  ...
  return (
    <div className="flex min-h-screen items-center justify-center bg-zinc-50 px-4">
      <div className="w-full max-w-2xl rounded-2xl border border-zinc-200 bg-white p-8 shadow-lg">
        <div className="mb-6 space-y-1">
          <div className="text-2xl font-semibold text-zinc-900">OnFire 安装向导</div>
          <div className="text-sm text-zinc-600">当前实例尚未初始化，请创建超级管理员并完成基础租户配置。</div>
        </div>
        <form className="grid grid-cols-1 gap-4 md:grid-cols-2" onSubmit={handleSubmit}>
          <div className="space-y-2 md:col-span-1">
            <div className="text-sm font-medium text-zinc-800">超级管理员邮箱</div>
            <Input required type="email" value={email} onChange={(e: ChangeEvent<HTMLInputElement>) => setEmail(e.target.value)} />
          </div>
          <div className="space-y-2 md:col-span-1">
            <div className="text-sm font-medium text-zinc-800">显示名称</div>
            <Input placeholder="可选，不填将使用邮箱前缀" value={displayName} onChange={(e: ChangeEvent<HTMLInputElement>) => setDisplayName(e.target.value)} />
          </div>
          <div className="space-y-2 md:col-span-1">
            <div className="text-sm font-medium text-zinc-800">密码</div>
            <Input required type="password" value={password} onChange={(e: ChangeEvent<HTMLInputElement>) => setPassword(e.target.value)} />
          </div>
          <div className="space-y-2 md:col-span-1">
            <div className="text-sm font-medium text-zinc-800">确认密码</div>
          ...
          <div className="space-y-2 md:col-span-2">
            <div className="text-sm font-medium text-zinc-800">租户名称</div>
            ...
            <p className="text-xs text-zinc-500">安装时至少需要创建一个租户，后续可在管理后台新增/编辑。</p>
          </div>
          {error && <div className="md:col-span-2 rounded-md bg-amber-50 px-3 py-2 text-sm text-amber-700">{error}</div>}
          {success && <div className="md:col-span-2 rounded-md bg-emerald-50 px-3 py-2 text-sm text-emerald-700">{success}</div>}
