...
- Each product can override four system email templates with custom HTML and a sandboxed live desktop/mobile preview.
...
A fresh local D1 successfully applied migrations `0000` through `0009`. Remote migration remains an explicit deployment operation.

## Verification

- `pnpm lint`: passing.
- `pnpm test`: 32 files, 150 tests passing, including proxy isolation, resolver URL/response limits, sealed-secret behavior, SLA state transitions, provider protocols, and RBAC scope.
...
- `wrangler check startup`: passing; final local CPU profile span was approximately 165 ms (the generated profile was removed after inspection).
...
- Playwright desktop/mobile visual regression: passed at 1440x900 and 390x844 in light/dark modes for management, email settings/templates, AI configuration, ticket detail, and search navigation.
