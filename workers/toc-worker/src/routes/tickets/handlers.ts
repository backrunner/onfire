...
export const listTemplates = async (env: Bindings, store: any, request: Request, query: any) => {
...
export const createTicket = async (env: Bindings, store: any, request: Request) => {
...
export const listTickets = async (env: Bindings, store: any, request: Request, query: any) => {
  const token = request.headers.get('authorization')?.replace('Bearer ', '');
  if (!token) return new Response('missing token', { status: 401 });
  const identity = await verifyJwt(token, store.env, store.db);
...
export const getTicket = async (env: Bindings, store: any, request: Request, id: string) => {
  const token = request.headers.get('authorization')?.replace('Bearer ', '');
  if (!token) return new Response('missing token', { status: 401 });
  const identity = await verifyJwt(token, store.env, store.db);
...
export const replyToTicket = async (env: Bindings, store: any, request: Request, id: string) => {
...
export const escalateTicket = async (env: Bindings, store: any, request: Request, id: string) => {
  const token = request.headers.get('authorization')?.replace('Bearer ', '');
  if (!token) return new Response('missing token', { status: 401 });
  const identity = await verifyJwt(token, store.env, store.db);
  const ticket = await store.db.query.tickets.findFirst({ where: eq(tickets.id, id) });
...
export const closeTicket = async (env: Bindings, store: any, request: Request, id: string) => {
...
export const reopenTicket = async (env: Bindings, store: any, request: Request, id: string) => {
  const body = (await request.json()) as { reason?: string; turnstileToken?: string };
  const token = request.headers.get('authorization')?.replace('Bearer ', '');
  if (!token) return new Response('missing token', { status: 401 });
  const identity = await verifyJwt(token, store.env, store.db);
