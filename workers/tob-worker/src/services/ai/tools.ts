 ...
 */
export const queryTickets = tool({
  ...
  execute: async (params, { context }: { context: ToolContext }) => {
 ...
 */
export const getTicketStatistics = tool({
    ...
    const breached = await context.db
      ...
      .where(
        and(
          ...
          sql`(json_extract(${tickets.sla}, '$.acceptBreached') = 1 OR json_extract(${tickets.sla}, '$.replyBreached') = 1)`
        )
      )
      .get();
 ...
 */
export const analyzeTicketTrends = tool({
  ...
  execute: async (params, { context }: { context: ToolContext }) => {
    ...
    const conditions: any[] = [gte(tickets.createdAt, dateFrom)];
    if (params.productId) {
      conditions.push(eq(tickets.productId, params.productId));
    }
...
export function getAgentTools(context: ToolContext) {
  return {
    ...
    generateReplyDraft: generateReplyDraft,
    getTicketStatistics: getTicketStatistics,
    analyzeTicketTrends: analyzeTicketTrends
  };
}
