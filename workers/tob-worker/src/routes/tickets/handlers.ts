...
export const listTickets = async (env: Bindings, store: any, user: any, query: any) => {
...
export const getTicketById = async (env: Bindings, store: any, user: any, id: string) => {
  const ctx = await resolveContext(env, user);
  assertPermission(ctx, 'ticket.read');
  let ticket = await store.db.query.tickets.findFirst({ where: eq(tickets.id, id) });
...
export const updateStatus = async (env: Bindings, store: any, user: any, id: string, body: { status?: TicketStatus; priority?: TicketPriority; reason?: string }) => {
  const ctx = await resolveContext(env, user);
...
export const assignTicket = async (env: Bindings, store: any, user: any, id: string, body: { assigneeId?: string }) => {
  const ctx = await resolveContext(env, user);
  const ticket = await store.db.query.tickets.findFirst({ where: eq(tickets.id, id) });
  if (!ticket) return new Response('not found', { status: 404 });
...
export const updatePriority = async (env: Bindings, store: any, user: any, id: string, body: { priority: TicketPriority; reason: string }) => {
  const ctx = await resolveContext(env, user);
...
export const closeTicket = async (env: Bindings, store: any, user: any, id: string, body: { reason?: string }) => {
  const ctx = await resolveContext(env, user);
...
export const bulkUpdateStatus = async (env: Bindings, store: any, user: any, body: { ids?: string[]; status?: TicketStatus; reason?: string }) => {
  const ctx = await resolveContext(env, user);
...
export const bulkAssign = async (env: Bindings, store: any, user: any, body: { ids?: string[]; assigneeId?: string }) => {
  const ctx = await resolveContext(env, user);
...
export const escalateTicket = async (env: Bindings, store: any, user: any, id: string, body: { reason?: string }) => {
  const ctx = await resolveContext(env, user);
  if (!body.reason) return new Response('reason required', { status: 400 });
  const ticket = await store.db.query.tickets.findFirst({ where: eq(tickets.id, id) });
  if (!ticket) return new Response('not found', { status: 404 });
...
export const replyToTicket = async (env: Bindings, store: any, user: any, id: string, body: { content: string; internal?: boolean }) => {
  const ctx = await resolveContext(env, user);
  const ticket = await store.db.query.tickets.findFirst({ where: eq(tickets.id, id) });
  if (!ticket) return new Response('not found', { status: 404 });
  assertPermission(ctx, 'ticket.write', { tenantId: ticket.tenantId as any, teamId: ticket.teamId as any, productId: ticket.productId as any });
