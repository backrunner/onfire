...
type TicketQuery = Record<string, string | undefined>;
...
export const listTickets = async (env: Bindings, store: AppStore, user: AuthUser | undefined, query: TicketQuery) => {
...
export const getTicketById = async (env: Bindings, store: AppStore, user: AuthUser | undefined, id: string) => {
  const ctx = await resolveContext(env, user);
  assertPermission(ctx, 'ticket.read');
  const ticketResult = await store.db.query.tickets.findFirst({ where: eq(tickets.id, id) });
  if (!ticketResult) return new Response('not found', { status: 404 });
  if (!ctx.tenantIds.includes(ticketResult.tenantId as string)) return new Response('forbidden', { status: 403 });
  let ticket = await maybeUpdateSla(store.db, ticketResult);
  ticket = await maybeAutoClose(store.db, ticket, ctx.user.id, Number(store.env.AUTO_CLOSE_REPLY_HOURS ?? 72));
  const replyRows = await store.db.select().from(replies).where(eq(replies.ticketId, id)).orderBy(replies.createdAt);
  const historyRows = await store.db.select().from(history).where(eq(history.ticketId, id)).orderBy(history.createdAt);
  const timeline = [
...
export const updateStatus = async (env: Bindings, store: AppStore, user: AuthUser | undefined, id: string, body: { status?: TicketStatus; priority?: TicketPriority; reason?: string }) => {
  const ctx = await resolveContext(env, user);
  const ticket = await store.db.query.tickets.findFirst({ where: eq(tickets.id, id) });
...
export const assignTicket = async (env: Bindings, store: AppStore, user: AuthUser | undefined, id: string, body: { assigneeId?: string }) => {
  const ctx = await resolveContext(env, user);
  const ticket = await store.db.query.tickets.findFirst({ where: eq(tickets.id, id) });
  if (!ticket) return new Response('not found', { status: 404 });
...
export const updatePriority = async (env: Bindings, store: AppStore, user: AuthUser | undefined, id: string, body: { priority: TicketPriority; reason: string }) => {
  const ctx = await resolveContext(env, user);
...
export const closeTicket = async (env: Bindings, store: AppStore, user: AuthUser | undefined, id: string, body: { reason?: string }) => {
  const ctx = await resolveContext(env, user);
...
export const bulkUpdateStatus = async (env: Bindings, store: AppStore, user: AuthUser | undefined, body: { ids?: string[]; status?: TicketStatus; reason?: string }) => {
  const ctx = await resolveContext(env, user);
...
export const bulkAssign = async (env: Bindings, store: AppStore, user: AuthUser | undefined, body: { ids?: string[]; assigneeId?: string }) => {
  const ctx = await resolveContext(env, user);
...
export const escalateTicket = async (env: Bindings, store: AppStore, user: AuthUser | undefined, id: string, body: { reason?: string }) => {
  const ctx = await resolveContext(env, user);
  if (!body.reason) return new Response('reason required', { status: 400 });
  const ticket = await store.db.query.tickets.findFirst({ where: eq(tickets.id, id) });
  if (!ticket) return new Response('not found', { status: 404 });
...
export const replyToTicket = async (env: Bindings, store: AppStore, user: AuthUser | undefined, id: string, body: { content: string; internal?: boolean }) => {
  const ctx = await resolveContext(env, user);
  const ticket = await store.db.query.tickets.findFirst({ where: eq(tickets.id, id) });
  if (!ticket) return new Response('not found', { status: 404 });
  assertPermission(ctx, 'ticket.write', { tenantId: ticket.tenantId as any, teamId: ticket.teamId as any, productId: ticket.productId as any });
